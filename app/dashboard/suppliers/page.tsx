...
function SupplierForm({ form, setForm, error, saving, onSave, onCancel, inline }: FormProps) {
  ...
  return (
    <div className={inline ? 'space-y-3' : 'space-y-4'}>
      <div className={`grid gap-3 ${inline ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2'}`}>
        <div>
        ...
        <div>
          ...
          <input value={form.notes} onChange={e => set('notes')(e.target.value)} placeholder="Materiales, tiempos, etc." className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-[#B03060] focus:ring-1 focus:ring-[#B03060]/20" />
        </div>
      </div>
